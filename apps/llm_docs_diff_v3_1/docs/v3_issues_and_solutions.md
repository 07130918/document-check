...
**問題**: 
画像内テキスト（漫画など）だけでなく、複数箇所でDocument Intelligenceがテキストを抽出できていない
...
**問題**: 
「補償内容」の「内」が「內」として誤認識される

**原因**:
- Azure Document IntelligenceのOCR精度の限界
- 日本語の異体字や類似文字の誤認識
...
**問題**: 
「有限会社矯正サービス」が下の「東京都～」とBBOXが結合している

**原因**:
- Azure Document Intelligence APIの仕様：`prebuilt-layout`モデルは近接するテキストを1つの行として認識する傾向
- 行の境界判定アルゴリズムが垂直方向の間隔を十分に考慮していない
- v3では行単位の処理のため、Azure APIが返す行をそのまま使用し、分割処理が未実装

**対応策**:
```python
def split_merged_lines(item: Dict) -> List[Dict]:
    """結合された行を分割"""
    text = item['text']
    bbox = item['bbox']
    
    # 住所パターンなどで分割位置を検出
    split_positions = detect_split_positions(text)
    
    if not split_positions:
        return [item]
    
    # BBOXを分割
    return create_split_items(text, bbox, split_positions)
```
