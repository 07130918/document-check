...
def main():
    ...
    if args.pdf1 and args.pdf2:
        ...
        # sougouデータセットを使用
        data_dir = Path("/home/dev/prj-ms-document-check.worktree/worktree1/data/data2/sougou")
    ...
    # タイムスタンプ付き出力ディレクトリ（v3フルバージョン、データセットごとに分ける）
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    base_output_name = f"llm_diff_test_v3.4_{timestamp}"
