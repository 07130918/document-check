...
from .azure_text_processing import group_japanese_characters_to_words, group_azure_words_to_lines
...
class DocumentAnalyzer:
    ...
    def analyze_documents(self, file1_path: str, file2_path: str) -> Tuple[ComparisonResult, bytes, bytes]:
        ...
        try:
            ...
            bbox_list1, azure_bbox_list1 = self._extract_bbox_data(pdf1_bytes)
            bbox_list2, azure_bbox_list2 = self._extract_bbox_data(pdf2_bytes)
            ...
            result = ComparisonResult(
                ...
                metadata={
                    ...
                    "azure_doc1_boxes": azure_bbox_list1,  # Azure抽出データ
                    "azure_doc2_boxes": azure_bbox_list2   # Azure抽出データ
                }
            )
    ...
    def _extract_bbox_data(self, pdf_bytes: bytes) -> Tuple[List[BBoxTextData], Optional[List[BBoxTextData]]]:
        """PDFからBBoxデータを抽出
        ...
        Returns:
            Tuple[通常抽出データ, Azure抽出データ（使用時のみ）]
        """
        azure_bbox_list = None
        ...
        if self.azure_service and settings.USE_AZURE_FOR_OCR:
            try:
                ...
                azure_bbox_list = self.azure_service.extract_layout_from_pdf(pdf_bytes)
                
                # 日本語の文字を意味的な単語にグループ化
                logger.info("Grouping Japanese characters into semantic words...")
                semantic_words = group_japanese_characters_to_words(azure_bbox_list)
                
                # 元のAzure抽出データも保持（メタデータとして）
                self._azure_raw_data = azure_bbox_list
                self._semantic_words = semantic_words
                
                bbox_list = semantic_words  # 意味的な単語を主として使用
            except Exception as e:
                logger.warning(f"Azure extraction failed, falling back to standard: {e}")
                # フォールバック: 通常のPDF処理
        ...
        if max_pages:
            ...
            # Azure抽出データも同様に制限
            if azure_bbox_list:
                azure_bbox_list = [bbox for bbox in azure_bbox_list if bbox["page"] < max_pages]
            
            return filtered_list, azure_bbox_list
        
        return bbox_list, azure_bbox_list
    ...
    def _estimate_reading_order(self, bbox_list: List[BBoxTextData]) -> List[BBoxTextData]:
        ...
        # Azure使用時は単語を行単位にグループ化
        if settings.USE_AZURE_FOR_OCR:
            logger.info("Grouping Azure words into lines...")
            from .azure_text_processing import group_azure_words_to_lines
            grouped_list = group_azure_words_to_lines(bbox_list)
        else:
            # 斜め文字や近接する要素をグループ化
