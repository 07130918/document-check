...
class ContentBasedDiffDetector:
    ...
    def detect_differences(self, 
        ...
        for text in matched_texts:
            ...
            # 位置の近さで1対1マッチングを行う
            matched_pairs = self._match_identical_texts_by_position(items1, items2)
            ...
            # マッチしなかったアイテムは残す
            doc1_by_text[text] = [item for i, item in enumerate(items1) if i not in matched_indices1]
            doc2_by_text[text] = [item for i, item in enumerate(items2) if i not in matched_indices2]
            
            # 空になったエントリは削除
            if not doc1_by_text[text]:
                del doc1_by_text[text]
            if not doc2_by_text[text]:
    ...
    def _match_identical_texts_by_position(self, items1: List[Dict[str, Any]], items2: List[Dict[str, Any]]) -> List[Tuple[Dict[str, Any], Dict[str, Any]]]:
        ...
        for item1 in items1:
            ...
            for item2 in items2:
                # ページ番号と座標から距離を計算
                page_diff = abs(item1.get('page', 0) - item2.get('page', 0))
                x_diff = abs(item1.get('x', 0) - item2.get('x', 0))
                y_diff = abs(item1.get('y', 0) - item2.get('y', 0))
                
                # ページが異なる場合は大きなペナルティ
                if page_diff > 0:
                    distance = page_diff * 10000 + (x_diff + y_diff)
                else:
        ...
        for i, j, dist in candidates:
            if i not in used_rows and j not in used_cols:
                matched_pairs.append((items1[i], items2[j]))
                used_rows.add(i)
                used_cols.add(j)
                
                # すべてマッチしたら終了
                if len(matched_pairs) == min(len(items1), len(items2)):
                    break
