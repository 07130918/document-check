#!/usr/bin/env python3
"""
LLM Diff Test v3-3 with LLM - 表検出機能付き差分検出
"""
import os
import sys
...
# v3-3のモジュールをインポート
import apps.llm_docs_diff_v3_3 as v3_3
from apps.llm_docs_diff_v3_3.services.azure_service_enhanced import AzureDocumentServiceEnhanced
from apps.llm_docs_diff_v3_3.services.llm_service import LLMService
from apps.llm_docs_diff_v3_3.core.text_preprocessing import TextPreprocessor
from apps.llm_docs_diff_v3_3.core.diff_detector_v3 import ContentBasedDiffDetector
from apps.llm_docs_diff_v3_3.core.table_aware_diff_detector import TableAwareDiffDetector
from apps.llm_docs_diff_v3_3.handlers.output_handler import OutputHandler
from apps.llm_docs_diff_v3_3.models.bbox_models import ComparisonResult, DiffResult, ChangeType
from apps.llm_docs_diff_v3_3.core.reading_order_v2 import ReadingOrderEstimatorV2
...
def parse_arguments():
    """コマンドライン引数の解析"""
    parser = argparse.ArgumentParser(description='LLM Diff Test v3-3 with LLM - 表検出機能付き差分検出')
...
def main():
    ...
    print("===== LLM Diff Test v3-3 with LLM =====")
    print(f"使用するPDFファイル:")
    ...
    if settings.USE_HIGH_RESOLUTION_OCR:
        if args.use_page_split:
            output_base_dir = settings.OUTPUT_DIR.parent / "llm_diff_test_v3-3_high_resolution_split_table" if args.use_table_detection else settings.OUTPUT_DIR.parent / "llm_diff_test_v3-3_high_resolution_split"
        else:
            output_base_dir = settings.OUTPUT_DIR.parent / "llm_diff_test_v3-3_high_resolution_table" if args.use_table_detection else settings.OUTPUT_DIR.parent / "llm_diff_test_v3-3_high_resolution"
    else:
        if args.use_page_split:
            output_base_dir = settings.OUTPUT_DIR.parent / "llm_diff_test_v3-3_standard_split_table" if args.use_table_detection else settings.OUTPUT_DIR.parent / "llm_diff_test_v3-3_standard_split"
        else:
            output_base_dir = settings.OUTPUT_DIR.parent / "llm_diff_test_v3-3_standard_table" if args.use_table_detection else settings.OUTPUT_DIR.parent / "llm_diff_test_v3-3_standard"
    ...
    if args.use_table_detection:
        # 表構造を考慮した差分検出
        print("表構造を考慮した差分検出を実行中...")
        # デバッグディレクトリを設定
        debug_dir = output_dir / "debug"
        table_aware_detector = TableAwareDiffDetector(debug_dir=debug_dir)
        diff_results, table_diffs = table_aware_detector.detect_differences_with_tables(
            pdf1_bytes, pdf2_bytes, ordered_list1, ordered_list2
        )
        ...
        if hasattr(table_aware_detector, 'tables1'):
            ...
            # 表情報をJSONとして保存
            import json
            page_tables_path = output_dir / "page_tables.json"
            with open(page_tables_path, 'w', encoding='utf-8') as f:
                json.dump({
                    'document1': tables1,
                    'document2': tables2
                }, f, ensure_ascii=False, indent=2)
            logger.info(f"Saved page tables to {page_tables_path}")
        
        # 表の差分を表示
        if table_diffs:
            print(f"\n表の差分: {len(table_diffs)}件")
            for td in table_diffs[:5]:  # 最初の5件を表示
                print(f"  - {td.summary}")
    else:
        # 従来の差分検出
    ...
    split_info = " + 2-way split" if args.use_page_split else ""
    table_info = " + table detection" if args.use_table_detection else ""
    print(f"結果は {relative_output} に保存されました ({resolution_type}{split_info}{table_info})")
