...
class OutputHandler:
    ...
    def _save_summary_report(self, result: ComparisonResult, base_name: str) -> Path:
        ...
        # 表の差分情報
        if result.metadata and result.metadata.get("table_detection_used") and result.metadata.get("table_diffs"):
            ...
            if table_diffs:
                ...
                for i, table_diff in enumerate(table_diffs[:5], 1):  # 最初の5件
                    report_lines.extend([
                        f"### {i}. 表の{table_diff['change_type']}",
                        f"- 表1インデックス: {table_diff.get('table_index1', 'N/A')}",
                        f"- 表2インデックス: {table_diff.get('table_index2', 'N/A')}",
                        f"- 概要: {table_diff['summary']}",
                    ])
                    
                    if table_diff.get('structure_changes'):
                        report_lines.append(f"- 構造変更: {', '.join(table_diff['structure_changes'])}")
                    
                    if table_diff.get('cell_changes'):
                        report_lines.append(f"- セル変更数: {len(table_diff['cell_changes'])}個")
                        # 最初の3つのセル変更を表示
                        for j, cell_change in enumerate(table_diff['cell_changes'][:3], 1):
                            if cell_change['change_type'] == 'modified':
                                report_lines.append(f"  - セル[{cell_change['row']},{cell_change['column']}]: '{cell_change['old_text'][:30]}...' → '{cell_change['new_text'][:30]}...'")
        ...
        if result.metadata and result.metadata.get("table_detection_used"):
            ...
            for table_diff in table_diffs:
                if table_diff['change_type'] == 'deletion':
                    ...
                    # 変更された表の個別セルをハイライト
                    table_idx1 = table_diff.get('table_index1')
                    table_idx2 = table_diff.get('table_index2')
                    
                    # セル変更情報を取得
                    cell_changes = table_diff.get('cell_changes', [])
                    
                    # 文書1の変更されたセルをハイライト
                    if table_idx1 is not None and table_idx1 in tables1_dict:
                        table1 = tables1_dict[table_idx1]
                    ...
                    if table_idx2 is not None and table_idx2 in tables2_dict:
                        table2 = tables2_dict[table_idx2]
